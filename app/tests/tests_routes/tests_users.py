...
class TestUsersRoutes:
    async def test_registration(self, test_client: AsyncClient, override_get_db_session, user_registration_data):
    ...
    async def test_me_active_user(self, test_client: AsyncClient, access_token, user_factory, override_get_db_session):
        user = await user_factory()
        token_data: TokenOutData = access_token(user_id=user.id)
        res = await test_client.get(
            "users/me", headers={"Authorization": f"{token_data.type} {token_data.access_token}"}
        )
    ...
    async def test_me_inactive_user(self, test_client: AsyncClient, access_token, user_factory):
    ...
    async def test_me_with_tasks(self, test_cred_client: AsyncClient, tasks_generating, user_active):
    ...
    async def test_me_none_user(self, test_client: AsyncClient, access_token):
    ...
    async def test_delete(self, test_client: AsyncClient, access_token, user_factory, override_get_db_session):
